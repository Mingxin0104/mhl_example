...
def make_train_valid_dfs():
    ...
    # mm_ids = np.arange(0, max_id)
    # np.random.seed(42)
    # valid_ids = np.random.choice(
    #     mm_ids, size=int(0.2 * len(mm_ids)), replace=False
    # )

    # train_ids = [id_ for id_ in mm_ids if id_ not in valid_ids]
    # dataframe['id'] = list(dataframe.index)
    # train_dataframe = dataframe[dataframe["id"].isin(train_ids)].reset_index(drop=True)
    # valid_dataframe = dataframe[dataframe["id"].isin(valid_ids)].reset_index(drop=True)
    # train_dataframe.to_csv('id_labels.csv')

    dataframe = pd.read_csv('id_labels.csv')
    dataframe["entity_id"] = dataframe["id"]
    train_dataframe = dataframe[:(3624-717)]
    valid_dataframe = dataframe[(3624-717):]

    return train_dataframe, valid_dataframe

def build_loaders(dataframe, mode):

    dataset = NodeMMDataset(
    ...
    for batch in tqdm_object:
        ...
        if args.prune:
            for name, module in model.named_modules():
                if 'mm_encoder.model' in name:
                    if args.mm_encoder_model == '':
...
def main():
    ...
    if args.mm_encoder_model == '':
        tokenizer = DistilBertTokenizer.from_pretrained(args.mm_encoder_tokenizer) # read distil-bert language model
    elif args.mm_encoder_model == '':
        tokenizer = BertTokenizer.from_pretrained(args.mm_encoder_tokenizer)  # read bert language model
    ...
    model = NodeMMCLModel(features).to(args.device)
    ...
    for epoch in range(args.pretrain_epochs):
        ...
        save_model_path = args.finetune_node_model_path #"./pretrain/{}_node_mm_{}.pt".format(args.dataset, my_time)
    ...
    with torch.no_grad():
        ...
        df = pd.read_csv(args.id_label_path)
        finetune_feature_path = args.finetune_feature_path #r'./finetune/{}_data/finetune_feature_{}.txt'.format(args.dataset,my_time)
        fo = open(finetune_feature_path, 'w', encoding='utf8')
        for i in range(df.shape[0]):
            # embed_ = model.mm_encoder.get_finetune_embed(df.loc[i, 'content'])
            embed_ = model.mm_encoder.forward(features['feature'])
            # np.savetxt(fo, np.array(embed_.detach().cpu()).reshape([1, 768]))
            if i % 1000 == 0:
                print("{} fine-tuned features have been generated!".format(i))

    save_embed_path = args.finetune_node_encoder_path #r'./finetune/{}_data/node_embed_{}.txt'.format(args.dataset,my_time)
